...
import { 
  ...
  CPCB_INDIA_STATION_DATABASE 
} from './src/data/cpcbNationalAqiData';
import { 
  NASA_MODIS_AQUA_PRODUCTS, 
  MODIS_AQUA_TELEMETRY_FEED 
} from './src/data/nasaModisAquaData';
import { 
  WHO_2021_AIR_QUALITY_GUIDELINES, 
  WHO_GLOBAL_CITIES_DATABASE 
} from './src/data/whoAqiDatabase';
import { 
  WAQI_GLOBAL_MONITORING_NETWORK 
} from './src/data/waqiGlobalData';
import { 
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Match against explicit user-configured ALLOWED_ORIGINS
    if (rawAllowedOrigins.includes(origin)) return callback(null, true);
    
    // Automatically permit Vercel deployment preview / production domains
    if (origin.endsWith('.vercel.app')) return callback(null, true);
    
    // Permit local development origins
    if (defaultDevOrigins.includes(origin)) return callback(null, true);
    
    // In non-production or open origin mode, reflect origin
    if (process.env.NODE_ENV !== 'production' || rawAllowedOrigins.length === 0) {
...
// ==========================================
// REAL-WORLD DATASET REGISTRY ENDPOINTS
// ==========================================
app.get('/api/datasets/cpcb', (req, res) => {
  res.json({
    source: 'Central Pollution Control Board (CPCB) India NAQI',
    url: 'https://cpcb.gov.in/National-Air-Quality-Index/',
    breakpoints: CPCB_NAQI_BREAKPOINTS,
    stations: CPCB_INDIA_STATION_DATABASE,
    timestamp: new Date().toISOString()
  });
});

app.get('/api/datasets/modis-aqua', (req, res) => {
  res.json({
    source: 'NASA MODIS Aqua Satellite & Google Earth Engine',
    url: 'https://developers.google.com/earth-engine/datasets/tags/aqua',
    products: NASA_MODIS_AQUA_PRODUCTS,
    telemetry: MODIS_AQUA_TELEMETRY_FEED,
    timestamp: new Date().toISOString()
  });
});

app.get('/api/datasets/who', (req, res) => {
  res.json({
    source: 'World Health Organization (WHO) Ambient Air Quality Database',
    url: 'https://www.who.int/data/gho/data/themes/air-pollution/who-air-quality-database',
    guidelines: WHO_2021_AIR_QUALITY_GUIDELINES,
    cities: WHO_GLOBAL_CITIES_DATABASE,
    timestamp: new Date().toISOString()
  });
});

app.get('/api/datasets/waqi', (req, res) => {
  res.json({
    source: 'World Air Quality Index (WAQI) & US EPA AQS',
    url: 'https://waqi.info/',
    network: WAQI_GLOBAL_MONITORING_NETWORK,
    timestamp: new Date().toISOString()
  });
});

app.get('/api/datasets/india-kaggle', (req, res) => {
  res.json({
    source: 'India Air Quality Index (AQI) Dataset 2023-2025',
    url: 'https://www.kaggle.com/datasets/saikiranudayana/india-air-quality-index-aqi-dataset-20232025',
    multiYearTrends: INDIA_AQI_MULTI_YEAR_DATASET,
    timestamp: new Date().toISOString()
  });
});
